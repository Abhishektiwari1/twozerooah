...
class WPCoupon_Coupons_Widget extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        $included_ids = [];
        if($instance['specific_post_ids'] and (!$instance['show_popular'])  and (!$instance['show_popular']) ) {
            $included_ids = $get_args['post__in'] = explode(',', $instance['specific_post_ids']);
        }
            ...
            <?php if ( ! $instance['hide_latest'] ) { ?>
                <div class="ajax-coupons coupon-tab-content latest-tab">
                    ...
                    <div class="store-listings st-list-coupons">
                        <?php
                        if( $included_ids){
                            # Respect the included ids
                            $reordered_posts = array_column($posts, 'ID');
                             foreach ($included_ids as $show_post_id) {
                                #Run the loop for included id so order will be maintained
                                if(false !== $founded_key = array_search($show_post_id, $reordered_posts)){
                                    #key found, show it
                                    wpcoupon_setup_coupon($posts[$founded_key], $current_link);
                                    get_template_part('loop/loop-coupon', $tpl_name);
                                }
                            }
                            #Special case ends
                        }else{#Normal work flow
                            ...
                            <div class="load-more wpb_content_element">
                                <a href="<?php echo next_posts($_max_page, false); ?>" class="ui button btn btn_primary btn_large"
                                   ...
                                   data-next-page="<?php echo($paged + 1); ?>" data-loading-text="<?php esc_attr_e('Loading...', 'wp-coupon'); ?>"><?php esc_html_e('Load More Coupons', 'wp-coupon'); ?>
                                    <i class="arrow circle outline down icon"></i></a>
                            </div>
                        <?php }
                    }
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'specific_post_ids' ); ?>"><?php esc_html_e( 'Specific Post IDs to include(only work if popular & ending soon is not enabled. ids should be comma separated with no space):', 'wp-coupon' ); ?></label>
            <input class="" id="<?php echo $this->get_field_id( 'specific_post_ids' ); ?>" name="<?php echo $this->get_field_name( 'specific_post_ids' ); ?>" type="text" value="<?php echo esc_attr( $instance['specific_post_ids'] ); ?>">
        </p>
        <p>
